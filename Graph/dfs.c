...
dfs_matrix_graph(matrix_graph_t* mg,
                 int node)
{
    stack* stack = st_create(sizeof(int));
    st_push(stack, &node);

    int* t_desc = calloc(1, mg->nodes * sizeof(int));
    DIE(!t_desc, "calloc() failed\n");
    int* t_fin = calloc(1, mg->nodes * sizeof(int));
    DIE(!t_fin, "calloc() failed\n");

    t_desc[node] = 1;

    while (!st_is_empty(stack)) {
        int x = *(int *)st_peek(stack);
        printf("%d ", x);
        st_pop(stack);

        for (int i = mg->nodes - 1; i >= 0; i--) {
			if (t_desc[i] == 0 && mg->matrix[x][i]) {
				t_desc[i] = 1;
				st_push(stack, &i);
			}
		}
