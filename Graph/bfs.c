...
lg_create(int nodes)
{
	list_graph_t* graph = malloc(sizeof(linked_list_t));
    DIE(!graph, "malloc() failed\n");

    graph->neighbors = malloc(nodes * sizeof(list_graph_t *));
...
{
	ll_node_t *iterator = graph->neighbors[src]->head;

    while (iterator) {
        if (*(int *)iterator->data == dest)
            return 1;
...
lg_remove_edge(list_graph_t* graph, int src, int dest)
{
    if (!lg_has_edge(graph, src, dest)) {
        printf("No edge to remove between the given nodes\n");
        return;
    }
    ...
    ll_node_t *rem_node = ll_remove_nth_node(graph->neighbors[src], index - 1);
    free(rem_node->data);
    free(rem_node);
}
...
void
bfs_list_graph(list_graph_t* lg, int node)
{
	queue_t *q = q_create(sizeof(int), lg->nodes);

    int *status = calloc(1, lg->nodes * sizeof(int));
    DIE(!status, "calloc() failed\n");

    int *parent = calloc(1, lg->nodes * sizeof(int));
    DIE(!parent, "calloc() failed\n");
    ...
    while (!q_is_empty(q)) {
        ...
        ll_node_t *iterator = lg->neighbors[x]->head;
        while (iterator) {
            int index = *(int *)iterator->data;
            if (status[index] == 0) {
                status[index] = 1;
                parent[index] = x;
                q_enqueue(q, iterator->data);
            }
            iterator = iterator->next;
        }
    }
