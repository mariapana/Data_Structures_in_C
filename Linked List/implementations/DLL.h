 ...
 * @return -> newly created doubly linked list (dll_t *)
 */
dll_t *ll_create(unsigned int data_size);
 ...
 * @param -> list = the list where we search for the node (dll_t *)
 * @param -> n = the index of the desired node
 * @return -> the node at the given index
 */
node_t *ll_get_nth_node(dll_t *list, unsigned int n);

/*
 * @brief -> adds a new node at index n in a given list
 * @param -> list = the list where the node is added (dll_t *)
 * @param -> n = the index (unsigned int)
 * @param -> new_data = info to be stored in the node (const void *)
 * @return -> the node at the given index
 */
void ll_add_nth_node(dll_t *list, unsigned int n, const void *data);

/*
 * @brief -> removes the nth node of a given list
 * @param -> list = the list from which we delete nodes (dll_t *)
 * @param -> n = the index of the node to be removed (unsigned int)
 * @return -> the now removed node
 */
node_t *ll_remove_nth_node(dll_t *list, unsigned int n);
